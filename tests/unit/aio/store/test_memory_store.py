...
async def test_add_honors_explicit_created_at_string():
    turns = MagicMock()
    turns.upsert_item = AsyncMock()
    store = AsyncMemoryStore(containers=_containers(turns=turns))
...
async def test_add_normalizes_datetime_created_at():
    turns = MagicMock()
    turns.upsert_item = AsyncMock()
    store = AsyncMemoryStore(containers=_containers(turns=turns))

    dt = datetime(2024, 3, 1, 12, 0, tzinfo=timezone.utc)
    await store.add(user_id="u1", role="user", content="hello", thread_id="t1", created_at=dt)
