...
class AsyncMemoryStore:
    ...
    async def _resolve_read_authz(self, ctx: SecurityContext, scope_keys: list[str]):
        return resolve_scope_access(ctx, scope_keys, "read")

    async def _resolve_scope_action(self, ctx: SecurityContext, scope_key: str, action: PermissionAction):
        return resolve_scope_access(ctx, [scope_key], action)

    async def _can_scope_action(self, ctx: SecurityContext, scope_key: str, action: PermissionAction) -> bool:
        return scope_key in (await self._resolve_scope_action(ctx, scope_key, action)).allowed_scopes

    async def _can_scope_any_action(
        self, ctx: SecurityContext, scope_key: str, actions: tuple[PermissionAction, ...]
    ) -> bool:
        for action in actions:
            if await self._can_scope_action(ctx, scope_key, action):
                return True
        return False
    ...
    async def _require_pin_agent_permission(self, ctx: SecurityContext, agent_scope: str) -> None:
        if await self._can_scope_any_action(ctx, agent_scope, ("assign", "write")):
            return
    ...
    async def _require_list_pins_permission(self, ctx: SecurityContext, agent_scope: str) -> None:
        if await self._can_scope_any_action(ctx, agent_scope, ("assign", "write", "read")):
            return
        raise ValidationError(
    ...
    async def _find_pin_memory_candidates(self, *, ctx: SecurityContext, memory_id: str) -> list[dict[str, Any]]:
    ...
    async def _read_pinned_memory(
        ...
        *,
        ...
        candidates = await self._find_pin_memory_candidates(ctx=ctx, memory_id=memory_id)
        if not candidates:
        ...
        for candidate in candidates:
            ...
            if not scope_key or not thread_id:
                continue
            resolution = await self._resolve_read_authz(ctx, [scope_key])
    ...
    async def pin_memory(
        ...
        Pass ``resource_type`` (``"memory"`` or ``"scope"``) to disambiguate a
        scope-shaped custom memory id; otherwise it is inferred from ``resource``.
        """
        if ctx is None:
        ...
        await self._require_pin_agent_permission(ctx, agent_scope)
        resource_scope_key: str | None = None
        if resource_type == "scope":
            resource_scope_key = normalized_resource
            resolution = await self._resolve_read_authz(ctx, [resource_scope_key])
            ...
            memory = await self._read_pinned_memory(ctx=ctx, memory_id=normalized_resource, include_superseded=True)
            if memory is None:
                raise ValidationError(f"pin_memory requires read permission on memory {normalized_resource!r}")
            candidates = await self._find_pin_memory_candidates(ctx=ctx, memory_id=normalized_resource)
            for candidate in candidates:
        ...
        response = await self._memories_container.upsert_item(body=self._prepare_doc(body))
    ...
    async def unpin_memory(
        ...
        try:
            await self._memories_container.delete_item(
                item=item_id,
    ...
    async def list_pins(self, agent_id: str, ctx: SecurityContext | None = None) -> list[dict[str, Any]]:
        ...
        agent_scope = agent_scope_key(agent_id)
        await self._require_list_pins_permission(ctx, agent_scope)
        query = (
    ...
    async def resolve_pinned_memories(
        self,
        *,
        agent_id: str,
        ctx: SecurityContext,
        ...
        pins = await self.list_pins(agent_id, ctx)
        ...
        for pin in pins:
            ...
            if resource_type == "memory":
                doc = await self._read_pinned_memory(
                    ctx=ctx,
                    memory_id=resource,
                    memory_types=types,
                ...
                docs = await self._read_pinned_scope(
                    ctx=ctx,
                    scope_key=resource,
                    memory_types=types,
                    limit=remaining,
    ...
    async def _require_promote_permission(self, ctx: SecurityContext, to_scope: str) -> None:
        if await self._can_scope_action(ctx, to_scope, "write"):
            return
        raise ValidationError(
            f"promote requires write permission on target scope {to_scope!r} for principal {ctx.principal!r}"
        )

    async def _read_memory_from_scope(self, memory_id: str, from_scope: str, tenant_id: str) -> dict[str, Any]:
        query = (
    ...
    async def promote(
        ...
        source_doc = await self._read_memory_from_scope(memory_id, from_scope, ctx.tenant_id)
        ...
        return await self.upsert_memory(promoted)
    ...
    async def list_promotion_candidates(
        ...
        *,
        ...
        read_resolution = await self._resolve_scope_action(ctx, scope, "read")
    ...
    async def approve_promotion(
        ...
        *,
        ...
        return await self.promote(memory_id, from_scope, to_scope, ctx)
    ...
    async def auto_promote_candidates(
        ...
        *,
        ...
        candidates = await self.list_promotion_candidates(
            ctx, from_scope=from_scope, min_confidence=confidence_threshold, top=top
        )
        ...
        for candidate in candidates:
            ...
            if is_pii_or_secret_flagged(candidate):
                ...
                continue
            has_write = await self._can_scope_action(ctx, target_scope, "write")
            if not has_write:
                results.append({"memory_id": candidate.get("id"), "status": "skipped", "reason": "permission_denied"})
                continue
            promoted = await self.promote(str(candidate["id"]), source_scope, target_scope, ctx)
    ...
    async def _require_shared_scope_access(
        self, ctx: SecurityContext, scope_key: str, action: PermissionAction
    ) -> None:
        scope_values_for_scope_key(scope_key, ctx.tenant_id)
        resolution = await self._resolve_scope_action(ctx, scope_key, action)
        if scope_key not in resolution.allowed_scopes:
            raise ValidationError(
    ...
    async def _read_shared_record_no_auth(self, *, ctx: SecurityContext, scope_key: str, key: str) -> dict[str, Any]:
        from azure.cosmos.exceptions import CosmosResourceNotFoundError

        record_key = self._validate_shared_record_key(key)
        try:
            doc = await self._memories_container.read_item(
                item=record_key,
            ...
            raise _wrap_cosmos_exception(
                exc, message=f"async get_shared_record read failed for key {record_key!r}: {exc}"
            ) from exc
    ...
    async def put_shared_record(
        ...
        *,
        ...
        await self._require_shared_scope_access(ctx, scope_key, "write")
        current: dict[str, Any] | None = None
        try:
            current = await self._read_shared_record_no_auth(ctx=ctx, scope_key=scope_key, key=key)
        ...
        try:
            response = await self._memories_container.upsert_item(body=body)
        except Exception as exc:
            raise _wrap_cosmos_exception(exc, message=f"async put_shared_record failed for key {key!r}: {exc}") from exc
        return response if isinstance(response, dict) else body
    ...
    async def get_shared_record(self, *, ctx: SecurityContext, scope_key: str, key: str) -> dict[str, Any]:
        ...
        await self._require_shared_scope_access(ctx, scope_key, "read")
        return await self._read_shared_record_no_auth(ctx=ctx, scope_key=scope_key, key=key)
    ...
    async def compare_and_swap_shared_record(
        ...
        *,
        ...
        try:
            response = await self._memories_container.replace_item(
                item=body["id"],
                body=body,
            ...
            raise _wrap_cosmos_exception(
                exc, message=f"async compare_and_swap_shared_record failed for key {key!r}: {exc}"
            ) from exc
        return response if isinstance(response, dict) else body
    ...
    async def update_shared_record(
        ...
        *,
        ...
        for _ in range(max_retries):
            ...
            try:
                return await self.compare_and_swap_shared_record(
                    ctx=ctx,
                    scope_key=scope_key,
                    key=key,
                    record=mutated,
    ...
    async def search(
        ...
        *,
        ...
        if ctx is not None and scope_keys:
            tenant_id = ctx.tenant_id
            authz_resolution = await self._resolve_read_authz(ctx, scope_keys)
            scope_keys = [scope_key for scope_key in scope_keys if scope_key in authz_resolution.allowed_scopes]
            if not scope_keys:
                return []

        async def run_one(scope_key: str | None) -> list[dict[str, Any]]:
            ...
            partition_key, _ = query_scope(scoped_user_id, thread_id, tenant_id, scope_key)
        ...
        if not scope_keys or len(scope_keys) == 1:
            return await run_one(scope_keys[0] if scope_keys else None)
        rows: list[dict[str, Any]] = []
        for scope_key in scope_keys:
            rows.extend(await run_one(scope_key))
